/* global liamandersonBgColors, liamandersonPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( liamandersonBgColors ).forEach( function( context ) {
					liamandersonGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function liamandersonGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'liamanderson-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#liamanderson-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( liamandersonPreviewEls[ context ] ) {
				_.each( liamandersonPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		liamandersonGenerateColorA11yPreviewStyles( 'content' );
		liamandersonGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
