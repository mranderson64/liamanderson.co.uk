/* global liamandersonBgColors, liamandersonColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( liamandersonBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( liamandersonBgColors[ context ].color ) {
						backgroundColorValue = liamandersonBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( liamandersonBgColors[ context ].setting ).get();
					}
					liamandersonSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( liamandersonBgColors ).forEach( function( context ) {
			wp.customize( liamandersonBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					liamandersonSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function liamandersonSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = liamandersonColor( backgroundColor, accentHue );
